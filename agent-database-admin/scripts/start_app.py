...
class ProcessManager:
    ...
    def clone_frontend_if_needed(self):
        ...
        branch = os.getenv("CHATBOT_CLONE_BRANCH", "main")
        for url in [
            os.getenv("CHATBOT_CLONE_REPO", "https://github.com/databricks/app-templates.git"),
            "https://github.com/databricks/app-templates.git",
            "git@github.com:databricks/app-templates.git",
        ]:
            try:
                subprocess.run(
                    ["git", "clone", "--filter=blob:none", "--sparse", "--branch", branch, url, "temp-app-templates"],
                    check=True,
                    capture_output=True,
                )
                break
    ...
    def apply_theme_overrides(self):
        ...
        """
        css_path = Path("e2e-chatbot-app-next/client/src/index.css")
        if not css_path.exists():
            return

        marker = "/* === agent-database-admin: dark red theme === */"
        try:
...
html {
  ...
  --foreground: #fce4e8;
  --card: #2a0a0e;
  --card-foreground: #fce4e8;
  --popover: #2a0a0e;
  --popover-foreground: #fce4e8;
  --tooltip: #fce4e8;
  --tooltip-foreground: #1a0608;

  --primary: #9e1d38;
  --primary-foreground: #ffffff;

  --secondary: #3a0f15;
  --secondary-foreground: #fce4e8;

  --muted: #3a0f15;
  --muted-foreground: #fbd0d8;

  --accent: #3a0f15;
  --accent-foreground: #fce4e8;
  ...
  --border-danger: rgba(232, 58, 95, 0.4);
  --border-warning: rgba(220, 98, 34, 0.3);
  --border-success: rgba(60, 164, 94, 0.3);
  --background-danger: rgba(232, 58, 95, 0.12);
  ...
  --sidebar-background: #2a0a0e;
  --sidebar-foreground: #fce4e8;
  --sidebar-primary: #9e1d38;
  --sidebar-primary-foreground: #ffffff;
  --sidebar-accent: #3a0f15;
  --sidebar-accent-foreground: #fce4e8;
  --sidebar-border: #4a1521;
  --sidebar-ring: #c82d4c;

  --action-default-background-hover: rgba(232, 58, 95, 0.08);
  --action-default-background-press: rgba(232, 58, 95, 0.16);
  --action-primary-background-hover: #c82d4c;
  --action-primary-background-press: #9e1d38;
  --action-tertiary-background-hover: rgba(232, 58, 95, 0.08);
  --action-tertiary-background-press: rgba(232, 58, 95, 0.16);
...
"""
        try:
            css_path.write_text(content + override)
            print("✓ Applied dark red theme overrides to chatbot UI")
        except OSError as exc:
            print(f"WARNING: Failed to apply theme overrides: {exc}")
