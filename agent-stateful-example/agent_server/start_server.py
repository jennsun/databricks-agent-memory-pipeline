...
from agent_server.agent import USER_LAKEBASE_CONFIG, AGENT_LAKEBASE_CONFIG
...
agent_server = AgentServer(
    ...
    db_instance_name=USER_LAKEBASE_CONFIG.instance_name,
    db_autoscaling_endpoint=USER_LAKEBASE_CONFIG.autoscaling_endpoint,
    db_project=USER_LAKEBASE_CONFIG.autoscaling_project,
    db_branch=USER_LAKEBASE_CONFIG.autoscaling_branch,
...
async def _lifespan(app):
    try:
        await run_lakebase_setup(USER_LAKEBASE_CONFIG, AGENT_LAKEBASE_CONFIG)
        ...
        if any(
            ...
            logger.error(
                ...
                exc,
                get_lakebase_access_error_message(USER_LAKEBASE_CONFIG.description),
            )
        else:
